...
template <typename T>
	...
	public:
		...
		{
			cout<<"key: " << _key << endl;
			cout<<"encrypted: " << _encrypted << endl;
			...
			if(dec == false && enc == false) //Process only if we've got an 'empty' SpaceCrypto OBJ
			{
				if(in_mode==HEX)
				{
					...
					_encrypted = normstr;
					cout<<"_encrypted = normstr: " << _encrypted << endl;
